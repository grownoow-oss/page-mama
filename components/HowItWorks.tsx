...
export default function HowItWorks() {
  ...
  return (
    <div id="how" className="relative max-w-7xl mx-auto px-4 sm:px-6 py-20 md:py-32">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] h-[80%] bg-[#FFCE0B]/5 blur-[100px] rounded-full pointer-events-none"></div>
      
      <div className="text-center mb-16 md:mb-20 relative z-10">
        <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full border border-yellow-100 bg-yellow-50 text-[#FFCE0B] font-bold tracking-wider text-xs uppercase mb-6">
          <span className="w-2 h-2 rounded-full bg-[#FFCE0B] animate-pulse"></span>
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="bg-white rounded-3xl p-8 sm:p-10 border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-black/5 transition-all duration-300 hover:-translate-y-1 group">
            <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-yellow-50 flex items-center justify-center mb-6 sm:mb-8 group-hover:scale-110 transition-transform duration-500">
              <step.icon className="w-7 h-7 sm:w-8 sm:h-8 text-[#FFCE0B]" />
            </div>
